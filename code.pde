...
void draw() {
  ...
  stroke(200);
  noFill();
  circle(logoX, logoY, max(50, logoZ * 2));
  ...
  if (dist(logoX, logoY, mouseX, mouseY) > max(50, logoZ)) {
    cursor(HAND);
  } else {
    cursor(ARROW);  
  }
...
void scaffoldControlLogic()
{
  ...
  rect(width*2/5, inchToPix(.6f), 100, 60);
  fill(0);
  text("Move", width*2/5, inchToPix(.7f));
  ...
  rect(width*3/5, inchToPix(.6f), 100, 60);
  fill(0);
  text("Rotate & \n Resize", width*3/5, inchToPix(.5f));  
}

void mousePressed()
{
    
  if (dist(width*2/5, inchToPix(.6f), mouseX, mouseY)< inchToPix(.7f)){
    option = 0;
    return;
  } else if (dist(width*3/5, inchToPix(.6f), mouseX, mouseY)< inchToPix(.5f)){
    option = 1;
    return;
  }
...
void mouseDragged()
{
  if ((option == 0) && (dist(width*2/5, inchToPix(.6f), mouseX, mouseY) >= inchToPix(.7f))){
    logoX = mouseX;
    logoY = mouseY;
  } else if ((option == 1) && (dist(width*3/5, inchToPix(.6f), mouseX, mouseY)>= inchToPix(.5f))) {  
